...
class Player:
    ...
    def __repr__(self):
        return "%d\t%d\t%d\t%.3f"%(self.wins, self.losses, self.wins+self.losses, float(self.wins) / (self.wins+self.losses))

def read_games():
    games = []
    response = requests.get('https://docs.google.com/spreadsheet/ccc?key=0ArCi1BfUMYrzdFhGdzc5NUJzS0xxX281QmF1RVJ4dmc&output=csv')
...
for n,p in sorted(players.items(), key=lambda p: p[1].wins, reverse=True):
    s+= "<tr align=\"center\"><td>%s<td>%d<td>%d<td>%d<td>%.3f"%(n, p.wins, p.losses, p.wins+p.losses, float(p.wins)/(p.wins+p.losses))
...
f.flush()
subprocess.call(['scp', f.name, 'dvl1@ssh.seas.wustl.edu:/home/research/dvl1/public_html/foosball/index.html'])
f.close()
